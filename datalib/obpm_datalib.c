...
void obpm_section_size(int size)
{
	_MAXSECTION  = size;
...
void obpm_material_size(int size)
{
...
void obpm_geometry_size(int size)
{
	_MAXGEOMETRY    = size;
...
void obpm_near1d_size(int size)
{
	_MAXNEAR1D      = size;
...
void obpm_near2d_size(int size)
{
	_MAXNEAR2D      = size;
...
void obpm_title(const char title[]) {strcpy(_title, title);}


void obpm_xsection1(double x) {_xsection[_xsections++] = x;}
void obpm_ysection1(double y) {_ysection[_ysections++] = y;}
void obpm_zsection1(double z) {_zsection[_zsections++] = z;}
void obpm_xdivision1(int n) {_xdivision[_xdivisions++] = (n != 0) ? n : 1;}
void obpm_ydivision1(int n) {_ydivision[_ydivisions++] = (n != 0) ? n : 1;}
void obpm_zdivision1(int n) {_zdivision[_zdivisions++] = (n != 0) ? n : 1;}


void obpm_xsection(int n, ...)
{
	int i;
	va_list ap;
	va_start(ap, n);
...
static void _obpm_material(int type, double d1, double d2, double d3, double d4, const char name[])
{
...
void obpm_material(double epsr, double esgm, double amur, double msgm, const char name[])
{
	_obpm_material(1, epsr, esgm, amur, msgm, name);
}


void obpm_material_dispersion(double einf, double ae, double be, double ce, const char name[])
{
	_obpm_material(2, einf, ae, be, ce, name);
}


void obpm_geometry_array(int material, int shape, const double g[])
{
...
void obpm_geometry(int material, int shape, double x1, double x2, double y1, double y2, double z1, double z2)
{
	double g[8];
	g[0] = x1;
	g[1] = x2;
	...
	obpm_geometry_array(material, shape, g);
}


void obpm_geometry_name(const char name[])
{
...
void obpm_feed(char dir, double x, double y, double z, double amp, double delay, double z0)
{
...
void obpm_planewave(double theta, double phi, int pol)
{
	_planewave = 1;
...
void obpm_load(char dir, double x, double y, double z, char type, double rcl)
{
...
void obpm_point(char dir, double x, double y, double z, const char prop[])
{
...
void obpm_rfeed(double rfeed) {_rfeed = rfeed;}
void obpm_pulsewidth(double pulsewidth) {_pulsewidth = pulsewidth;}
void obpm_timestep(double timestep) {_timestep = timestep;}


void obpm_pml(int l, double m, double r0)
{
	_abc = 1;
	_pml_l = l;
	_pml_m = m;
...
void obpm_pbc(int pbcx, int pbcy, int pbcz)
{
	_pbcx = pbcx;
	_pbcy = pbcy;
	_pbcz = pbcz;
...
void obpm_frequency1(double fstart, double fend, int div)
{
	_f1start = fstart;
	_f1end   = fend;
	_f1div   = div;
...
void obpm_frequency2(double fstart, double fend, int div)
{
	_f2start = fstart;
	_f2end   = fend;
	_f2div   = div;
...
void obpm_solver(int maxiter, int nout, double converg)
{
	_maxiter = maxiter;
	_nout    = nout;
	_converg = converg;
...
void obpm_matchingloss(int i0)
{
	_matchingloss = i0;
}


void obpm_plotiter(int i0)
{
	_plotiter = i0;
}


void obpm_plotfeed(int i0)
{
	_plotfeed = i0;
}


void obpm_plotpoint(int i0)
{
	_plotpoint = i0;
}


void obpm_plotsmith(int i0)
{
	_plotsmith = i0;
}


void obpm_plotzin(int scale, double min, double max, int div)
{
	if      (scale == 1) {
		_plotzin = 1;
		_zinscale.user = 0;
...
void obpm_plotyin(int scale, double min, double max, int div)
{
	if      (scale == 1) {
		_plotyin = 1;
		_yinscale.user = 0;
...
void obpm_plotref(int scale, double min, double max, int div)
{
	if      (scale == 1) {
		_plotref = 1;
		_refscale.user = 0;
...
void obpm_plotspara(int scale, double min, double max, int div)
{
	if      (scale == 1) {
		_plotspara = 1;
		_sparascale.user = 0;
...
void obpm_plotcoupling(int scale, double min, double max, int div)
{
	if      (scale == 1) {
		_plotcoupling = 1;
...
void obpm_plotfar0d(double theta, double phi, int scale, double min, double max, int div)
{
	if      (scale == 1) {
		_plotfar0d = 1;
...
void obpm_freqdiv(int freqdiv)
{
	_freqdiv = freqdiv;
}


void obpm_plotfar1d(char dir, int div, double angle)
{
...
void obpm_far1dstyle(int i0)
{
	_far1dstyle = i0;
}


void obpm_far1dcomponent(int i0, int i1, int i2)
{
...
void obpm_far1ddb(int i0)
{
	_far1dscale.db = (i0 == 0) ? 0 : 1;
}


void obpm_far1dnorm(int i0)
{
	_far1dnorm = i0;
}


void obpm_far1dscale(double min, double max, int div)
{
	_far1dscale.user = 1;
	_far1dscale.min = min;
	_far1dscale.max = max;
...
void obpm_plotfar2d(int divtheta, int divphi)
{
	_far2ds = 1;
...
void obpm_far2dcomponent(int i0, int i1, int i2, int i3, int i4, int i5, int i6)
{
...
void obpm_far2ddb(int i0)
{
	_far2dscale.db = (i0 == 0) ? 0 : 1;
}


void obpm_far2dscale(double min, double max, int div)
{
	_far2dscale.user = 1;
	_far2dscale.min = min;
	_far2dscale.max = max;
...
void obpm_far2dobj(double obj)
{
	_far2dobj = obj;
}


void obpm_plotnear1d(const char component[], char dir, double p1, double p2)
{
...
void obpm_near1ddb(int i0)
{
	_near1dscale.db = (i0 == 0) ? 0 : 1;
}


void obpm_near1dnoinc(int i0)
{
	_near1dnoinc = i0;
}


void obpm_near1dscale(double min, double max, int div)
{
	_near1dscale.user = 1;
...
void obpm_plotnear2d(const char component[], char dir, double p)
{
...
void obpm_near2ddim(int i0, int i1)
{
	_near2ddim[0] = i0;
	_near2ddim[1] = i1;
}
...
void obpm_near2dframe(int i0)
{
	_near2dframe = i0;
}


void obpm_near2ddb(int i0)
{
	_near2dscale.db = (i0 == 0) ? 0 : 1;
}


void obpm_near2dscale(double min, double max)
{
	_near2dscale.user = 1;
...
void obpm_near2dcontour(int i0)
{
	_near2dcontour = i0;
}


void obpm_near2dobj(int i0)
{
	_near2dobj = i0;
}


void obpm_near2dnoinc(int i0)
{
	_near2dnoinc = i0;
}


void obpm_near2dzoom(double p0, double p1, double p2, double p3)
{
	_near2dizoom = 1;
	_near2dpzoom[0] = p0;
	_near2dpzoom[1] = p1;
...
void obpm_outdata(const char fn[])
{
	FILE  *fp;
