...
def main(argv):
    # ロゴ
    version = 'OpenBPM (Python) Version 4.2.0'

    # 計算モード
    GPU = 0    # 0=CPU/1=GPU
    VECTOR = 0  # VECTOR: 0=OFF/1=ON

    # 入力データファイル名(obpm.pyからの相対パス)
    obpm_in = "python.obpm"
    #obpm_in = "../data/sample/1st_sample.obpm"
    #obpm_in = "../data/benchmark/benchmark100.obpm"
    ...
    # 引数処理(引数があるときは引数優先)
    if len(argv) > 1:
        GPU, VECTOR, thread, Npx, Npy, Npz, f_dtype, prompt, obpm_in \
        = _args(argv, version)
    #print(GPU, VECTOR, thread, Npx, Npy, Npz, f_dtype, prompt, obpm_in)
    ...
    if io:
        ...
        = sol.input_data.read(obpm_in, Parm)
...
def _args(argv, version):
    ...
    usage = 'Usage : python obpm.py [-cpu|-gpu] [-n <thread>] [-p <x> <y> <z>] [-no-vector|-vector] [-single|-double] <datafile>'
    GPU = 0
    VECTOR = 0
    thread = 1
    Npx = Npy = Npz = 1
    ...
    return GPU, VECTOR, thread, Npx, Npy, Npz, f_dtype, prompt, obpm_in
