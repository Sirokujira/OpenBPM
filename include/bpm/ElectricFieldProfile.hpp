...
public:
    // Constructor
    ElectricFieldProfile(double Lx = 1.0, double Ly = 1.0)
    ...
    void initializeEfromFunction(const std::function<std::complex<float>(float, float, float, const std::vector<std::complex<float>>&)>& hFunc, const std::vector<std::complex<float>>& nParameters = {}, int Nz = 1) {
        if (Nz <= 0) {
